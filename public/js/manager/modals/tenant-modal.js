...
(function () {
    ...
    class TenantModalManager {
        ...
        async loadInitialData() {
            try {
                ...
                const unitsResponse = await PM.APIClient.get('/api/manager/units/available');
                if (unitsResponse.success) {
                    tenantModalState.availableUnits = unitsResponse.data.data || [];
                }
            } catch (error) {
        ...
        initializeFormHandlers() {
            ...
            if (addTenantForm && !addTenantForm.dataset.initialized) {
                ...
                addTenantForm.addEventListener('submit', this.handleAddTenant.bind(this));
            }
            ...
            if (assignUnitForm && !assignUnitForm.dataset.initialized) {
                ...
                assignUnitForm.addEventListener('submit', this.handleAssignUnit.bind(this));
            }
        }
        ...
        async loadAvailableUnits() {
            try {
                const response = await PM.APIClient.get('/api/manager/units/available');
                if (response.success) {
        ...
        async handleAddTenant(e) {
            ...
            try {
                ...
                const response = await PM.APIClient.post('/api/manager/tenants', formData);

                if (response.success) {
                    PM.NotificationManager.success('Tenant created successfully!');
                    if (formData.sendCredentials) {
                        PM.NotificationManager.info('Login credentials sent to tenant email');
                    }
        ...
        async handleAssignUnit(e) {
            ...
            try {
                ...
                if (response.success) {
                    PM.NotificationManager.success('Unit assigned successfully!');
        ...
        openAddTenantModal() {
            ...
            PM.ModalManager.openModal('addTenantModal');
        ...
        openSendCredentialsModal(tenantId) {
            ...
            if (dataContainer) {
                if (nameEl) nameEl.textContent = dataContainer.dataset.tenantName || '-';
                if (emailEl) emailEl.textContent = dataContainer.dataset.tenantEmail || '-';
                if (unitEl) {
                    const unitNumber = dataContainer.dataset.unitNumber;
                    unitEl.textContent = unitNumber ? `Unit: ${unitNumber}` : 'No unit assigned';
                }
            } else {
                ...
                if (card) {
                    ...
                    if (unitEl) {
                        unitEl.textContent = unitInfo ? `Unit: ${unitInfo}` : 'No unit assigned';
                    }
                } else {
            ...
            PM.ModalManager.openModal('sendCredentialsModal');
            document.dispatchEvent(
                new CustomEvent('tenantModalOpen', {
        ...
        openAssignUnitModal(tenantId) {
            ...
            if (card) {
                const name =
                    card.getAttribute('data-name') || card.querySelector('h3')?.textContent;
                const email =
                    card.getAttribute('data-email') ||
                    card.querySelector('.email-text')?.textContent;
            ...
            this.loadAvailableUnits();
            PM.ModalManager.openModal('assignUnitModal');
            document.dispatchEvent(
                new CustomEvent('tenantModalOpen', {
        ...
        openDeleteTenantModal(tenantId) {
            ...
            PM.ModalManager.openModal('deleteTenantModal');
            document.dispatchEvent(
                new CustomEvent('tenantModalOpen', {
        ...
        closeAddTenantModal() {
            PM.ModalManager.closeModal('addTenantModal');
            const form = document.getElementById('addTenantForm');
            if (form) form.reset();
        }

        closeSendCredentialsModal() {
            PM.ModalManager.closeModal('sendCredentialsModal');
        ...
        closeAssignUnitModal() {
            PM.ModalManager.closeModal('assignUnitModal');
        ...
        closeDeleteTenantModal() {
            PM.ModalManager.closeModal('deleteTenantModal');
            tenantModalState.selectedTenantId = null;
        }
        ...
        openSuccessModal(title, message, callback) {
            ...
            PM.ModalManager.openModal('successModal');
        }

        closeSuccessModal() {
            PM.ModalManager.closeModal('successModal');
        ...
        async confirmSendCredentials() {
            ...
            try {
                ...
                if (response.success) {
                    PM.NotificationManager.success('Credentials sent successfully!');
        ...
        async confirmDeleteTenant() {
            ...
            if (confirmation !== 'DELETE') {
                PM.NotificationManager.error('Please type DELETE to confirm');
                return;
            }
            ...
            try {
                ...
                if (response.success) {
                    PM.NotificationManager.success('Tenant deleted successfully');
    ...
    window.sendCredentials = (tenantId) => getInstance().openSendCredentialsModal(tenantId);
    window.assignUnitToTenant = (tenantId) => getInstance().openAssignUnitModal(tenantId);
    window.deleteTenant = (tenantId) => getInstance().openDeleteTenantModal(tenantId);

    window.closeAddTenantModal = () => getInstance().closeAddTenantModal();
    window.closeSendCredentialsModal = () => getInstance().closeSendCredentialsModal();
    window.closeAssignUnitModal = () => getInstance().closeAssignUnitModal();
    window.closeDeleteTenantModal = () => getInstance().closeDeleteTenantModal();

    window.generatePassword = () => getInstance().generatePassword();
    window.generateNewPassword = () => getInstance().generateNewPassword();
    window.confirmSendCredentials = () => getInstance().confirmSendCredentials();
