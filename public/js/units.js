...
// Initialize when document is ready - update your CasaConnect.ready function
CasaConnect.ready(() => {
...
function populateEditForm(unitData) {
    ...
    setTimeout(() => {
        ...
        if (form.querySelector('[name="unitNumber"]')) {
            form.querySelector('[name="unitNumber"]').value =
                unitData.unitNumber || "";
        }
        ...
        if (form.querySelector('[name="bathrooms"]')) {
            form.querySelector('[name="bathrooms"]').value =
                unitData.bathrooms || "";
        }
        if (form.querySelector('[name="squareFeet"]')) {
            form.querySelector('[name="squareFeet"]').value =
                unitData.squareFeet || "";
        }
        if (form.querySelector('[name="monthlyRent"]')) {
            form.querySelector('[name="monthlyRent"]').value =
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    throw new Error(
                        response.error || "Failed to assign tenant"
                    );
                }
            } catch (error) {
                CasaConnect.NotificationManager.error(error.message);
...
function assignTenant(unitId) {
    ...
    if (unitCard) {
        ...
        const unitDetails =
            unitCard.querySelector(".unit-info-grid").textContent;

        document.getElementById("assignUnitId").value = unitId;
        document.getElementById("assignUnitNumber").textContent = unitNumber;
        document.getElementById("assignUnitDetails").textContent =
            "Available for assignment";
    }

    CasaConnect.ModalManager.openModal("assignTenantModal");
}

function closeAssignTenantModal() {
    CasaConnect.ModalManager.closeModal("assignTenantModal");
...
function filterUnits() {
    ...
    const visibleUnits = document.querySelectorAll(
        '.unit-card-full:not([style*="display: none"])'
    );
