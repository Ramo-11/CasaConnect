...
const TenantServiceRequest = {
    ...
    async submitRequest(form) {
        ...
        if (missing.length > 0) {
            PM.NotificationManager.error('Please fill in all required fields');
        ...
        if (!selectedMethod) {
            PM.NotificationManager.error('Please select a payment method');
            this.processing = false;
            return;
        }
        ...
        if (photosInput?.files?.length) {
            if (photosInput.files.length > 3) {
                PM.NotificationManager.error('Max 3 photos allowed');
                this.processing = false;
                return;
            }
            ...
            if (tooBig) {
                PM.NotificationManager.error('Each photo must be under 5MB.');
                this.processing = false;
                return;
            }
            if (total > 15 * 1024 * 1024) {
                PM.NotificationManager.error('Total photo size exceeds 15MB.');
                this.processing = false;
                return;
            }
        }
        ...
        try {
            ...
            const response = await PM.APIClient.post('/api/tenant/service-request', formData);

            if (response.success) {
                PM.NotificationManager.success('Service request submitted successfully!');
            ...
            PM.NotificationManager.error(error.message || 'Failed to submit request');
        } finally {
            this.processing = false;
    ...
    async openServiceRequestModal() {
        ...
        PM.ModalManager.openModal('serviceRequestModal');
            ...
            <div class="payment-methods-list">
                ${methods
                    ...
                    <label class="payment-method-option">
                        <input type="radio" name="servicePaymentMethodId" value="${
    ...
    closeServiceRequestModal() {
        PM.ModalManager.closeModal('serviceRequestModal');
    ...
    async loadRequestHistory() {
        try {
            const response = await PM.APIClient.get('/api/tenant/service-requests');
            if (response.success) {
            ...
            <div class="request-card">
                ...
                ${
                    request.notes && request.notes.length > 0
                        ? `
                    <button class="btn-link" onclick="TenantServiceRequest.toggleNotes('${
                        request.id
                    }')">
