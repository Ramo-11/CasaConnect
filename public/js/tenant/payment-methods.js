...
const PaymentMethodManager = {
    ...
    setupPaymentTypeToggle() {
        const radios = document.querySelectorAll('input[name="newPaymentType"]');
        radios.forEach((r) =>
            r.addEventListener('change', (e) => {
                ...
                const accountHolder = document.querySelector('input[name="accountHolder"]');
                const routingNumber = document.querySelector('input[name="routingNumber"]');
    ...
    async savePaymentMethod(form) {
        ...
        try {
            const type = form.querySelector('input[name="newPaymentType"]:checked').value; // "card" | "ach"

            // 1) Server creates a SetupIntent for the chosen type
            const siResp = await PM.APIClient.post('/api/tenant/payment-methods/setup-intent', {
                type,
            });
            if (!siResp?.success)
                throw new Error(siResp?.message || 'Failed to create setup intent');
            const clientSecret =
                siResp?.clientSecret ??
            ...
            if (typeof clientSecret !== 'string' || !clientSecret.includes('_secret_')) {
                throw new Error('Bad SetupIntent response (missing client_secret).');
            }
            ...
            if (type === 'card') {
                const cardholderName =
                    document.getElementById('cardholderName')?.value || undefined;
                if (!this.cardElement) throw new Error('Stripe card element not ready');

                confirmResult = await this.stripe.confirmCardSetup(clientSecret, {
                    payment_method: {
                ...
                confirmResult = await this.stripe.confirmUsBankAccountSetup(clientSecret, {
                    payment_method: {
            ...
            const saveResp = await PM.APIClient.post('/api/tenant/payment-methods', {
                type,
                paymentMethodId,
            });

            if (!saveResp?.success)
                throw new Error(saveResp?.message || 'Failed to save payment method');

            PM.NotificationManager.success(
                type === 'ach' ? 'Bank added. Watch for micro‑deposits.' : 'Payment method saved.'
            );
            this.closeModal();
            this.loadPaymentMethods();
        } catch (error) {
            PM.NotificationManager.error(`Error saving payment method: ${error.message}`);
        } finally {
    ...
    async loadPaymentMethods() {
        try {
            const response = await PM.APIClient.get('/api/tenant/payment-methods');
        ...
        container.innerHTML = methods
            ...
            <div class="payment-method-item">
                ...
                <div class="method-actions">
                    ${
                        !method.isDefault && (method.type === 'card' || method.verified)
    ...
    async setDefaultPaymentMethod(methodId) {
        try {
            ...
            if (response.success) {
                PM.NotificationManager.success('Default payment method updated');
                this.loadPaymentMethods();
            } else {
    ...
    async removePaymentMethod(methodId) {
        ...
        try {
            const response = await PM.APIClient.delete(`/api/tenant/payment-method/${methodId}`);

            if (response.success) {
                PM.NotificationManager.success('Payment method removed');
                this.loadPaymentMethods();
            } else {
                throw new Error(response.message);
            }
        } catch (error) {
            PM.NotificationManager.error(`Failed to remove payment method: ${error.message}`);
        }
    },

    openModal() {
        PM.ModalManager.openModal('addPaymentMethodModal');
        setTimeout(() => {
    ...
    closeModal() {
        PM.ModalManager.closeModal('addPaymentMethodModal');
        const form = document.getElementById('addPaymentMethodForm');
        if (form) form.reset();
