...
const TenantPayment = {
    ...
    async processPayment(form) {
        ...
        try {
            ...
            const response = await PM.APIClient.post('/api/tenant/payment/process', {
                paymentMethodId: selectedMethod.value,
                amount: amount,
            });

            if (response.success) {
                PM.NotificationManager.success('Payment processed successfully!');
    ...
    displaySavedMethods(methods) {
        ...
        container.innerHTML = methods
            ...
            <div class="saved-method" data-method-id="${method._id}">
                <input type="radio" name="savedMethod" id="method-${method._id}" value="${
                    method._id
                }">
    ...
    async openPaymentModal() {
        ...
        if (amountElement) {
            document.getElementById('paymentAmount').textContent =
                amountElement.textContent.replace('$', '');
        }
        ...
        if (!response.success || !response.data.data || response.data.data.length === 0) {
            ...
            PM.ModalManager.openModal('paymentModal');
            return;
        }
    ...
    closePaymentModal() {
        PM.ModalManager.closeModal('paymentModal');
    },

    async refreshPaymentHistory() {
        try {
            const response = await PM.APIClient.get('/api/tenant/payment-history');
            if (response.success) {
