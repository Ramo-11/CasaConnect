...
const TenantDashboard = {
    ...
    initializeTabs() {
        ...
        const lastTab = PM.StorageHelper.get('tenantLastTab');
        if (lastTab) {
    ...
    switchTab(targetTab) {
        ...
        // Save preference
        PM.StorageHelper.set('tenantLastTab', targetTab);
    ...
    async loadPaymentStatus() {
        try {
            const response = await PM.APIClient.get('/api/tenant/payment-status');
            if (response.success) {
    ...
    setupEventListeners() {
        ...
        document.addEventListener('keydown', (e) => {
            // ESC to close modals
            if (e.key === 'Escape') {
                PM.ModalManager.closeAllModals();
    ...
    filterPayments(filterValue) {
        ...
        rows.forEach((row) => {
            const shouldShow =
                filterValue === 'all' || row.getAttribute('data-type') === filterValue;
            row.style.display = shouldShow ? '' : 'none';
        });

        // Handle empty state
        const visibleRows = document.querySelectorAll(
            '.payment-table tbody tr:not([style*="display: none"])'
        );
    ...
    viewServicePhotos(requestId, photosOverride) {
        ...
        if (!photos || photos.length === 0) {
            ...
            PM.NotificationManager.info('No photos available for this request');
            return;
        }
    ...
    async cancelServiceRequest(requestId) {
        ...
        try {
            const response = await PM.APIClient.delete(`/api/tenant/service-request/${requestId}`);

            if (response?.success) {
                PM.NotificationManager.success('Service request cancelled successfully');
            ...
            PM.NotificationManager.error(error?.message || 'Failed to cancel service request');
        }
    },
    ...
    async loadDocuments() {
        ...
        try {
            const response = await PM.APIClient.get('/api/tenant/documents');
            if (response.success) {
            ...
            <div class="document-item">
                ...
                <div class="document-info">
                    ...
                    <span class="document-meta">
                        ${doc.type} • ${this.formatFileSize(doc.size)} • ${new Date(
                    doc.createdAt
                ).toLocaleDateString()}
                    </span>
                </div>
                <div class="document-actions">
                    <button class="btn-icon" onclick="DocumentManager.viewDocument('${
                        doc._id
                    }')" title="View">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn-icon" onclick="DocumentManager.downloadDocument('${
                        doc._id
                    }')" title="Download">
