...
exports.uploadDocument = async (req, res) => {
    try {
        ...
        const folder =
            normalizedType === 'lease' ? 'documents/leases' : `documents/${normalizedType}`;
        ...
        if (relatedModel === 'User' && relatedId) {
            ...
            if (activeLease) {
                ...
                if (activeLease.additionalTenants?.length)
                    sharedUsers.push(...activeLease.additionalTenants);
                document.sharedWith = sharedUsers;
                logger.info(
                    `Document shared with unit's current tenants: ${sharedUsers.join(', ')}`
                );
            }
        }
...
exports.getDocuments = async (req, res) => {
    try {
        ...
        // For restricted managers, filter documents
        if (userRole === 'restricted_manager') {
            ...
            const accessibleTenantIds = activeLeases.map((l) => l.tenant);
            const accessibleLeaseIds = activeLeases.map((l) => l._id);

            // Build complex filter for documents
            filter.$or = [
                // Documents related to accessible units
                { 'relatedTo.model': 'Unit', 'relatedTo.id': { $in: accessibleUnits } },
                // Documents related to accessible tenants
                { 'relatedTo.model': 'User', 'relatedTo.id': { $in: accessibleTenantIds } },
                // Documents related to accessible leases
                { 'relatedTo.model': 'Lease', 'relatedTo.id': { $in: accessibleLeaseIds } },
            ];
        }
...
exports.deleteDocument = async (req, res) => {
    try {
        ...
        if (
            document.type === 'lease' &&
            document.relatedTo?.model === 'Lease' &&
...
exports.viewDocument = async (req, res) => {
    try {
        ...
        if (!hasAccess) {
            logger.warn(
                `Unauthorized document access attempt by user ${userId} for document ${documentId}`
            );
...
exports.downloadDocument = async (req, res) => {
    try {
        ...
        if (!hasAccess) {
            logger.warn(
                `Unauthorized document download attempt by user ${userId} for document ${documentId}`
            );
            return res.status(403).json({
                success: false,
                message: 'Access denied',
